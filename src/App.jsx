          ...
          <header className="flex items-center justify-between px-6" style={headerStyle}>
            ...
            <nav className="flex items-center gap-2">
              ...
              <button
                ...
                title={isAdmin
                  ? (lang === 'en' ? 'Logged in as Admin — click to log out' : 'Als Admin eingeloggt — klicken zum Ausloggen')
                  : (lang === 'en' ? 'Log in as Admin' : 'Als Admin einloggen')}
                style={{
                  marginLeft: 8,
                  width: 36,
                  height: 36,
                  borderRadius: '50%',
                  border: `1px solid ${isAdmin ? c.accent : c.accent + '40'}`,
                  background: isAdmin ? `${c.accent}20` : 'transparent',
                  cursor: 'pointer',
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                  color: isAdmin ? c.accent : (isDark ? 'rgba(255,255,255,0.5)' : 'rgba(0,0,0,0.4)'),
                  transition: 'all 0.2s',
                  position: 'relative',
                  flexShrink: 0,
                }}
                onMouseEnter={e => {
                  ...
                  e.currentTarget.style.background = `${c.accent}25`
                }}
                onMouseLeave={e => {
                  e.currentTarget.style.borderColor = isAdmin ? c.accent : c.accent + '40'
                  e.currentTarget.style.color = isAdmin ? c.accent : (isDark ? 'rgba(255,255,255,0.5)' : 'rgba(0,0,0,0.4)')
                  e.currentTarget.style.background = isAdmin ? `${c.accent}20` : 'transparent'
                }}
              >
                {isAdmin ? <LogOut size={15} /> : <User size={15} />}
                {/* Green dot indicator when admin */}
                {isAdmin && (
                  <span style={{
                    position: 'absolute',
                    top: 1,
                    ...
                    borderRadius: '50%',
                    background: c.accent,
                    border: `1.5px solid ${isDark ? '#0a0a0a' : '#f0efe8'}`,
                  }} />
                )}
              </button>
            </nav>
