...
export default function HomePage() {
  ...
  const fgSubtle = isDark ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.3)'
  const borderColor = isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.08)'
  const cardBg = isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)'
  const cardBorder = isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.08)'
  const gridColor = isDark ? 'rgba(16,185,129,0.04)' : 'rgba(16,185,129,0.06)'
  const tickerBg = isDark ? 'rgba(16,185,129,0.03)' : 'rgba(16,185,129,0.04)'
  const fadeLeft = isDark ? 'linear-gradient(to right, #080c0a, transparent)' : 'linear-gradient(to right, #ffffff, transparent)'
  const fadeRight = isDark ? 'linear-gradient(to left, #080c0a, transparent)' : 'linear-gradient(to left, #ffffff, transparent)'
    ...
    <div style={{ fontFamily: "'DM Serif Display', Georgia, serif", background: bg, color: fg, overflowX: 'hidden', transition: 'background 0.3s, color 0.3s' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .framework-pill { padding: 8px 20px; font-family: 'DM Mono', monospace; font-size: 12px; letter-spacing: 0.05em; transition: all 0.2s; }
      ...
      <section ref={heroRef} style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '120px 80px 80px', overflow: 'hidden' }}>
        ...
        <div className="orb" style={{ width: 600, height: 600, background: 'rgba(16,185,129,0.12)', top: -100, right: -100 }} />
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 900 }}>
          <div className="hero-badge" style={{ marginBottom: 32 }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 14px', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.3)', color: '#10b981', fontFamily: "'DM Mono', monospace", fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
          ...
          <h1 className="hero-title" style={{ fontSize: 'clamp(3rem, 7vw, 6.5rem)', lineHeight: 1.02, letterSpacing: '-0.03em', marginBottom: 32, fontWeight: 400, color: fg }}>
            {t.heroTitle1}<br /><em style={{ color: '#10b981' }}>{t.heroTitle2}</em>
          </h1>
          ...
          <div className="hero-cta" style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            <button onClick={() => navigate('/record')} style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '16px 36px', background: '#10b981', color: '#080c0a', fontFamily: "'DM Mono', monospace", fontSize: 13, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', border: 'none', cursor: 'pointer', transition: 'background 0.2s, transform 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = '#34d399'} onMouseLeave={e => e.currentTarget.style.background = '#10b981'}>{t.enterPlatform}</button>
            <button onClick={() => navigate('/reports')} style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '16px 36px', background: 'transparent', color: fg, fontFamily: "'DM Mono', monospace", fontSize: 13, fontWeight: 400, letterSpacing: '0.08em', textTransform: 'uppercase', border: `1px solid ${isDark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.2)'}`, cursor: 'pointer', transition: 'border-color 0.2s' }}>{t.viewReports}</button>
          </div>
        </div>

        <div style={{ position: 'absolute', right: 80, top: '50%', transform: 'translateY(-50%)', animation: 'float 6s ease-in-out infinite', opacity: 0.8 }}>
          <div style={{ padding: 24, border: '1px solid rgba(16,185,129,0.2)', background: isDark ? 'rgba(8,12,10,0.8)' : 'rgba(255,255,255,0.9)', backdropFilter: 'blur(20px)', minWidth: 200 }}>
        ...
        <div style={{ overflow: 'hidden', position: 'relative' }}>
          ...
          <div className="services-track">
            {[...t.services, ...t.services].map((service, i) => (
              <div key={i} className="service-card" style={{ border: `1px solid ${cardBorder}`, background: cardBg }}>
                ...
                <div style={{ position: 'absolute', bottom: 36, right: 36 }}>
                  <span className="mono" style={{ fontSize: 11, color: 'rgba(16,185,129,0.6)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{t.learnMore}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ padding: '100px 80px', background: statsBg, borderTop: `1px solid ${borderColor}`, borderBottom: `1px solid ${borderColor}` }}>
        <div data-id="how" className={`reveal${visible.how ? ' visible' : ''}`}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}>
            ...
            <div>
              <div style={{ border: `1px solid ${cardBorder}`, padding: 40, background: isDark ? 'rgba(8,12,10,0.6)' : 'rgba(255,255,255,0.8)' }}>
      ...
      <section style={{ padding: '100px 80px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div data-id="cta" className={`reveal${visible.cta ? ' visible' : ''}`} style={{ position: 'relative', zIndex: 1, textAlign: 'center', maxWidth: 700, margin: '0 auto' }}>
          ...
          <button onClick={() => navigate('/record')} style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '18px 48px', background: '#10b981', color: '#080c0a', fontFamily: "'DM Mono', monospace", fontSize: 14, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', border: 'none', cursor: 'pointer' }}>{t.launchPlatform}</button>
        </div>
      </section>
