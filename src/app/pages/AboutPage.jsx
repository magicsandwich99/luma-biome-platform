...
export default function AboutPage() {
  ...
  const { accent, bg, fg, fgMuted, cardBg, cardBorder } = c

  const sponsorBorder = isDark ? 'rgba(16,185,129,0.2)' : 'rgba(124,58,237,0.25)'
  const sponsorBg = isDark ? 'rgba(16,185,129,0.03)' : 'rgba(124,58,237,0.04)'
    ...
    <div style={{ fontFamily: "'DM Serif Display', Georgia, serif", background: bg, color: fg, minHeight: '100vh', padding: '80px', transition: 'background 0.3s, color 0.3s' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <h1 style={{ fontSize: 'clamp(2.5rem, 5vw, 4rem)', fontWeight: 400, letterSpacing: '-0.02em', marginBottom: 16, lineHeight: 1.05 }}>
          {t.title1}<br /><em style={{ color: accent }}>{t.title2}</em>
        </h1>
        <p className="mono" style={{ fontSize: 13, color: fgMuted, maxWidth: 540, lineHeight: 1.8, marginBottom: 80, fontWeight: 300 }}>{t.desc}</p>

        <div className="mono" style={{ fontSize: 11, color: fgMuted, letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 24 }}>{t.studentTeam}</div>
        ...
        <div className="mono" style={{ fontSize: 11, color: fgMuted, letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 24 }}>{t.projectSponsor}</div>
        <div style={{ maxWidth: 320 }}>
          <div className="sponsor-card" style={{ borderRadius: 2, border: `1px solid ${sponsorBorder}`, background: sponsorBg }}>
            ...
            <div style={{ padding: '20px 20px 24px' }}>
              ...
              <div className="mono" style={{ fontSize: 11, color: accent, letterSpacing: '0.05em', opacity: 0.7 }}>{sponsor.role}</div>
            </div>
          </div>
        </div>
      </div>
