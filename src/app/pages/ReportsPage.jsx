...
export default function ReportsPage() {
  ...
  function handleExport(exportFormat) {
    ...
    const data = selectedSensor.dataHistory.map(entry => ({ timestamp: entry.timestamp instanceof Date ? entry.timestamp.toISOString() : entry.timestamp, value: entry.value, sensor: selectedSensor.name, unit: selectedSensor.unit }))
    const filename = `${selectedSensor.name.replace(/\s+/g, '_')}_${Date.now()}`
    if (exportFormat === 'csv') {
      const csv = [['Timestamp', 'Value', 'Sensor', 'Unit'], ...data.map(row => [row.timestamp, row.value, row.sensor, row.unit])].map(row => row.join(',')).join('\n')
  ...
  function calculateStats() {
    if (!selectedSensor || !selectedSensor.dataHistory?.length) return { avg: 0, min: 0, max: 0, trend: 'stable' }
  ...
  const chartData = selectedSensor?.dataHistory?.length ? selectedSensor.dataHistory.slice(-parseInt(timeRange)).map(entry => ({ date: format(entry.timestamp instanceof Date ? entry.timestamp : new Date(entry.timestamp), 'MMM dd'), value: parseFloat(entry.value.toFixed(2)) })) : []
  const sensorColor = selectedSensor ? categoryColors[selectedSensor.category] || '#10b981' : '#10b981'
  const tooltipStyle = { contentStyle: { backgroundColor: '#1e293b', border: '1px solid #374151', borderRadius: '8px', color: '#e5e7eb' }, labelStyle: { color: '#9ca3af' } }
    ...
    if (chartType === 'bar') return (
      <ResponsiveContainer width="100%" height={350}>
        <BarChart {...commonProps}>
          <defs><linearGradient id="barGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={sensorColor} stopOpacity={0.8} /><stop offset="100%" stopColor={sensorColor} stopOpacity={0.4} /></linearGradient></defs>
          <CartesianGrid strokeDasharray="3 3" stroke="#374151" opacity={0.3} />
          <XAxis dataKey="date" stroke="#9ca3af" fontSize={12} />
          <YAxis stroke="#9ca3af" fontSize={12} />
          <Tooltip {...tooltipStyle} />
    ...
    return (
      <ResponsiveContainer width="100%" height={350}>
        <AreaChart {...commonProps}>
          <defs><linearGradient id="areaGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={sensorColor} stopOpacity={0.6} /><stop offset="100%" stopColor={sensorColor} stopOpacity={0.05} /></linearGradient></defs>
          <CartesianGrid strokeDasharray="3 3" stroke="#374151" opacity={0.3} />
          <XAxis dataKey="date" stroke="#9ca3af" fontSize={12} />
          <YAxis stroke="#9ca3af" fontSize={12} />
          <Tooltip {...tooltipStyle} />
  ...
  const trendLabel = { up: t.up, down: t.down, stable: t.stable }

  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        <h1 className="font-semibold text-gray-100 mb-1">{t.title}</h1>
        <p className="text-sm text-gray-400">{t.desc}</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-6">
        <Card className="p-6 bg-gradient-to-br from-green-500/10 to-green-600/5 border-green-500/30">
          <div className="flex items-center justify-between mb-2"><span className="text-sm text-gray-300">{t.average}</span><TrendingUp className="w-4 h-4 text-green-400" /></div>
        ...
        <Card className="p-6 bg-gradient-to-br from-blue-500/10 to-blue-600/5 border-blue-500/30">
          <div className="flex items-center justify-between mb-2"><span className="text-sm text-gray-300">{t.minimum}</span><TrendingDown className="w-4 h-4 text-blue-400" /></div>
        ...
        <Card className="p-6 bg-gradient-to-br from-orange-500/10 to-orange-600/5 border-orange-500/30">
          <div className="flex items-center justify-between mb-2"><span className="text-sm text-gray-300">{t.maximum}</span><TrendingUp className="w-4 h-4 text-orange-400" /></div>
        ...
        <Card className="p-6 bg-gradient-to-br from-purple-500/10 to-purple-600/5 border-purple-500/30">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-gray-300">{t.trend}</span>
          ...
          <div className={`text-2xl font-bold ${stats.trend === 'up' ? 'text-green-400' : stats.trend === 'down' ? 'text-red-400' : 'text-gray-400'}`}>{trendLabel[stats.trend]}</div>
          <div className="text-xs text-gray-400 mt-1">{t.last} {timeRange} {t.days}</div>
        </Card>
      </div>

      <Card className="p-6 mb-6 bg-[#131621] border-gray-800">
        <div className="flex items-center justify-between mb-6">
          <h2 className="font-semibold text-gray-100">{t.visualization}</h2>
      ...
      <Card className="p-6 bg-[#131621] border-gray-800 mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="font-semibold text-gray-100 mb-1">{t.exportData}</h2>
            <p className="text-sm text-gray-400">{t.exportDesc}</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button onClick={() => handleExport('json')} variant="outline" disabled={!selectedSensor} className="border-gray-700 text-gray-300 hover:bg-gray-800">
              <Download className="w-4 h-4 mr-2" /> {t.exportJson}
            </Button>
          </div>
        </div>
        {selectedSensor && (
          <div className="bg-[#1e293b] rounded-lg p-4 border border-gray-700">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div><span className="text-gray-400">{t.sensor}:</span><p className="font-medium text-gray-100">{selectedSensor.name}</p></div>
              <div><span className="text-gray-400">{t.category}:</span><p className="font-medium text-gray-100">{selectedSensor.category}</p></div>
              <div><span className="text-gray-400">{t.dataPoints}:</span><p className="font-medium text-gray-100">{selectedSensor.dataHistory?.length || 0}</p></div>
              <div>
                <span className="text-gray-400">{t.status}:</span>
                <Badge className={`mt-1 block w-fit ${selectedSensor.status === 'active' ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-gray-500/20 text-gray-400 border border-gray-500/30'}`}>{selectedSensor.status}</Badge>
              </div>
            </div>
          </div>
        )}
      ...
      <Card className="p-6 bg-[#131621] border-gray-800">
        <h2 className="font-semibold text-gray-100 mb-4">{t.overview}</h2>
              ...
              <Card key={sensor.id} className="p-4 bg-[#1e293b] border-gray-700 hover:border-gray-600 transition-all">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <Badge style={{ backgroundColor: `${color}20`, color, borderColor: `${color}50` }}>{sensor.category}</Badge>
                  </div>
                  <Badge className={sensor.status === 'active' ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-gray-500/20 text-gray-400 border border-gray-500/30'}>{sensor.status}</Badge>
                </div>
                {sensor.lastReading && <div className="mb-2"><span className="text-2xl font-bold" style={{ color }}>{sensor.lastReading}</span><span className="text-sm text-gray-400 ml-1">{sensor.unit}</span></div>}
                <div className="text-xs text-gray-400">{sensor.dataHistory?.length || 0} {t.dataPoints}</div>
              </Card>
            )
          })}
        </div>
